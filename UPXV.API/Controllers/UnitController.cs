...
public sealed class UnitController : ControllerBase
{
   ...
   public IActionResult Get (int nid) => _service.Get(nid).Either(Ok,
      ...
      {
         EntityNotFoundException<Unit> e => NotFound(e.Message),
         Exception e => Problem(e.Message)
      });

   [HttpPost]
   public IActionResult Create (UnitCreateDTO dto) => _service.Create(dto).Either(Ok,
      problem => problem switch
      {
   ...
   [HttpPut]
   public IActionResult Update (UnitUpdateDTO dto) => _service.Update(dto).Either(Ok,
      problem => problem switch
      {
         ValidationException e => BadRequest(e.Errors),
         EntityNotFoundException<Unit> e => NotFound(e.Message),
